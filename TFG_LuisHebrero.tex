...
\begin{mdframed}
    \begin{exmp}
        ...
        \begin{equation}\label{eq:SolTo1dEq}
            ...
            \right.
        \end{equation}
        En la serie de experimentos numéricos, utilizamos una red residual con la siguiente configuración $1$-$256$-$256$-$256$-$1$.  Así, la función de coste de esta red neuronal vendrá dada por la siguiente función.
        \begin{equation*} 
            \mathcal{L}(\boldsymbol{\theta}; \mathcal{T}) = w_f \mathcal{L}_f(\boldsymbol{\theta}; \mathcal{T}_f) + w_b \mathcal{L}_b(\boldsymbol{\theta}; \mathcal{T}_b),
        \end{equation*}
        donde
        ...
        Bajo esta configuración, obtenemos la función de red $\hat{u}(\mathbf{x};\theta)$ numéricamente a través del método PINN después del proceso de entrenamiento. 
        Trazamos la solución $\hat{u}(\mathbf{x};\theta)$ y la comparamos con la solución débil en la Figura \ref{fig:deviation}. 
...
Como hemos visto, las PINN presentan errores elevados en algunos problemas. Estos errores pueden ser debidos a varios factores. Uno de los problemas fundamentales de las PINN es que no garantizan la unicidad de la solución. A diferencia de los métodos numéricos tradicionales, las PINNs no garantizan soluciones únicas, ya que trabajan resolviendo problemas de optimización no convexos, los cuales generalmente no tienen una única solución. No hay ninguna garantía de que la red neuronal converja a la solución correcta, y de hecho, en muchos casos, no lo hace, es más, como ocurre con muchos algoritmos de IA, converge a una solución errónea con la misma convicción (o error) que para la solución real.


El problema principal radica en la formulación fuerte de las ecuaciones diferenciales, la cual exige que la red neuronal se ajuste a la solución exacta en todos los puntos del dominio. Este enfoque puede ser demasiado restrictivo y dificultar la optimización de la función de coste. 
