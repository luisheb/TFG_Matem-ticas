...
Un resultado relevante para justificar el uso de funciones de activación no lineales es el siguiente es el propuesto por Pinkus \cite[Theorem 3.1]{Pinkus}. Este resultado es para redes neuronales de una sola capa.
...
Como habíamos comentado en la Sección \ref{sec:RedesNeuronales}, las redes neuronales son capaces de aproximar cualquier función, por lo que, en teoría, pueden aproximar cualquier solución de una ecuación diferencial. De este modo, las PINN van a aprovechar esto, definiendo la aproximación de nuestra solución $\hat{u}$, como la salida de nuestra función red neuronal. Así, la función en la Figura \ref{fig:PIN} sería la aproximación de la solución de la ecuación diferencial.
\begin{figure}[htbp]
    \centering
    \label{fig:PIN}
...
\marginnote{Creo que habría \\que ser más \\coherente\\ con la notación introducida}
\begin{equation} 
...
\end{equation}
Con condición de frontera $u(\mathbf{x}) = g(\mathbf{x})$ en $\partial \Omega$. 
La idea de las PINN es entrenar una red neuronal para que aproxime la solución de la ecuación diferencial. Para ello, se define una red neuronal $u_{\theta}(\mathbf{x})$ con pesos $\theta$ que aproxima la solución de la ecuación diferencial. Así, la función red neuronal se entrena para que aproxime la solución de la ecuación diferencial en todo el dominio. Para ello, se define una función de coste que mida el error de la red neuronal en aproximar la ecuación diferencial y las condiciones de frontera.

Para apli
El siguiente paso, sería definir una función de coste. Es decir, una función que nos diga como de buena es nuestra aproximación. En el caso de las PINN, la función de coste se define como la suma de dos términos. El primero de ellos, se encarga de verificar la ecuación diferencial, es decir, que la función red neuronal aproxima la ecuación diferencial en todo el dominio. El segundo término, se encarga de verificar las condiciones de frontera e iniciales.
\begin{equation}
...
\subsection{Limitaciones de las PINN}
