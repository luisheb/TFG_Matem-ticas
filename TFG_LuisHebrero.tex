...
\begin{mdframed}
    \begin{exmp}
        ...
        \end{equation*}
        Para resolver este problema, he implementado el método Deep Ritz en Python usando PyTorch. La red neuronal utilizada tiene la siguiente configuración: $2$-$8$-$8$-$1$. En la Figura \ref{fig:poisson_square} se muestra la solución obtenida con $\lambda = 500$. No obstante, siguiendo la intuición que desarrollado en el ejemplo en una dimensión, si usamos $\lambda = 10^{-5}$ esperamos obtener una aproximación mucho peor. Efectivamente, al realizar esta aproximación, el resultado que obtenemos no se parece nada a la solución real de la EDP. Podemos ver el resultado de este método en la Figura \ref{fig:poisson_square_lambda_1e-5}.
...
No obstante, en muchos casos, no es posible encontrar una solución en forma fuerte, es decir, una función que cumpla la ecuación en un conjunto finito de puntos cumpla las condiciones de frontera. En estos casos, se recurre a métodos alternativos, como la formulación débil de la ecuación, que permite encontrar una solución en un espacio de funciones más amplio. A continuación, vemos un ejemplo que motiva la necesidad de recurrir a la formulación débil de una ecuación.
