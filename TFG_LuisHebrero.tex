...
Para pasar de forma fuerte a debil, lo primero que hacemos es 
...
El método de los elementos finitos (MEF) es una técnica numérica para resolver ecuaciones diferenciales parciales, como las ecuaciones diferenciales elípticas con condiciones de frontera de Dirichlet. Este método es ampliamente utilizado en ingeniería y ciencias aplicadas porque permite aproximar soluciones en dominios complejos.
...
\begin{defin}
    ...
    \begin{align*}
        ...
        f\in C(\overline{\Omega})&\label{eq:condiciones_EDP_eliptica_f}
    \end{align*}
    Y además, cumple la siguiente propiedad de simetría, es decir, que los coeficientes $a_{ij}$ son simétricos, es decir, 
    \begin{equation}\label{eq:simetria_a}
        a_{ij} = a_{ji}.
    \end{equation}
\end{defin}
Resulta, que bajo esta condición, el problema elíptico autoadjunto se puede formular como un problema de minimización. En concreto, se puede demostrar que la solución de la ecuación \ref{eq:EDP_eliptica_autoadjunta} es el mínimo de la siguiente función
\begin{equation}
    J(u) = \frac{1}{2}\int_\Omega \left( \sum_{i,j=1}^{n} a_{ij}(x)\frac{\partial u}{\partial x_i}\frac{\partial u}{\partial x_j} + \sum_{i=1}^{n} b_i(x)\frac{\partial u}{\partial x_i} + c(x)u - f(x)u\right)dx.
\end{equation}
