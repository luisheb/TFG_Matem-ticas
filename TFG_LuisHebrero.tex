...
Como se ve en la Figura \ref{fig:RedNeuronal}, la entrada en nuestra función está representada por los dos círculos de la izquierda, que representan los valores de entrada $x_1$ y $x_2$, siendo así la entrada $x\in\mathbb{R}^2$. Estos valores se multiplican por unos pesos ($W^{[2]}$) y se suman a un sesgo $b^{[2]}$. Finalmente, la salida de esta capa se obtiene aplicando una función de activación $\sigma$. De modo que, los valores que ``llegan'' a la segunda capa de nuestra red neuronal serán de la forma
\begin{equation*}
...
\begin{figure}
    ...
    \caption{Clasificación final aprendida por la red neuronal}
...
Un resultado relevante para justificar el uso de funciones de activación no lineales es el siguiente propuesto por Pinkus \cite[Theorem 3.1]{pinkus1999approximation}, formulado para redes neuronales de una sola capa.
...
\end{teor}
De forma más intuitiva, cualquier función $f \in C(\mathbb{R}^n)$, se puede aproximar tan bien como se quiera con una red neuronal de una sola capa si y solo si la función de activación no es polinómica. Este resultado muestra que las funciones de activación no lineales son necesarias, ya que sin ellas habría muchas funciones que no podríamos aproximar.
...
\begin{dem}
    ...
    \end{itemize}
    Se comprueban así las condiciones (a), (b) y (c) del teorema de Lax-Milgram y concluimos que existe una única solución débil en $H^1_0(\Omega)$ para la ecuación \eqref{eq:ProblemaDebilFormaBilineal}.\qed
\end{dem}
...
Como se comentó en la Sección \ref{sec:RedesNeuronales}, las redes neuronales tienen la capacidad de aproximar cualquier función continua definida en un dominio compacto. Por ello, pueden emplearse para aproximar soluciones de ecuaciones diferenciales. Las PINNs se basan en esta idea y definen la aproximación de la solución $u$ como la salida de la red neuronal $\hat{u}_\theta$, donde $\theta \in \mathbb{R}^{\# \mathcal{N}}$ representa los pesos y sesgos (con $\# \mathcal{N}$ indicando el número total de parámetros libres). La función mostrada en la Figura \ref{fig:PIN} corresponde, por tanto, a la aproximación de la solución de la ecuación diferencial.
\begin{figure}[htbp]
    \centering
...
Con las PINNs nosotros queremos encontrar una función $\hat{u}_\theta$ que minimice el residuo en $\Omega$ y que cumpla las condiciones de contorno en $\partial\Omega$. Para ello, lo que queremos es la $\hat{u}_\theta$ que haga que las siguientes expresiones sean mínimas:
\begin{align*}
    &\int_{\Omega} \left( -\sum_{i,j=1}^{n} \frac{\partial}{\partial x_j}\left( a_{ij}(\mathbf{x})\frac{\partial \hat{u}_\theta}{\partial x_i}\right) + \sum_{i=1}^{n} b_i(\mathbf{x})\frac{\partial \hat{u}_\theta}{\partial x_i} + c(\mathbf{x})\hat{u}_\theta - f(\mathbf{x}) \right)^2, \\ 
    &\int_{\partial\Omega} \left( \hat{u}_\theta (\mathbf{x}) - g(\mathbf{x})\right)^2.
\end{align*}
...
\begin{equation}
    \label{eq:Lf_Lb}
    \begin{split}
        \mathcal{L}_f(\boldsymbol{\theta}; \mathcal{T}_f) &= \frac{1}{|\mathcal{T}_f|} \sum_{\mathbf{x} \in \mathcal{T}_f} | -\sum_{i,j=1}^{n} \frac{\partial}{\partial x_j}\left( a_{ij}(\mathbf{x})\frac{\partial \hat{u}_\theta}{\partial x_i}\right) \\ &+ \sum_{i=1}^{n} b_i(\mathbf{x})\frac{\partial \hat{u}_\theta}{\partial x_i} + c(\mathbf{x})\hat{u}_\theta - f(\mathbf{x}) |^2, \\ 
        \mathcal{L}_b(\boldsymbol{\theta}; \mathcal{T}_b) &= \frac{1}{|\mathcal{T}_b|} \sum_{\mathbf{x} \in \mathcal{T}_b} \left| \hat{u}_\theta (\mathbf{x}) - g(\mathbf{x})\right|^2.
    \end{split}
\end{equation}
...
Una ventaja fundamental del uso de redes neuronales en este contexto es la diferenciación automática, una técnica numérica muy potente que facilita la obtención de las derivadas de $\hat{u}_\theta$ con respecto a sus entradas de forma exacta y eficiente.
...
\begin{mdframed}
    \begin{exmp}
        ...
        u(-1) = 0, \quad u(1) = 0.
        \end{equation*}
        En esta ecuación la solución exacta es  conocida $ u(x) = \sin(\pi x)$. Para resolver este problema con una PINN, utilizamos la librería de python DeepXDE \cite{lu2021deepxde}, la cual tiene implementada la funcionalidad necesaria para entrenar una red neuronal que aproxime EDPs. Así, comenzamos definiendo el intervalo en el que se encuentra el dominio y los puntos de colocación
...
\end{lstlisting}
    ...
    donde
    \begin{align*}
        \mathcal{L}_f(\boldsymbol{\theta}; \mathcal{T}_f) &= \frac{1}{|\mathcal{T}_f|} \sum_{\mathbf{x} \in \mathcal{T}_f} \left| \frac{\partial^2\hat{u}_\theta}{\partial x^2} +\pi^2 \sin(\pi x)\right|^2, \\ 
        \mathcal{L}_b(\boldsymbol{\theta}; \mathcal{T}_b) &= \frac{1}{|\mathcal{T}_b|} \sum_{\mathbf{x} \in \mathcal{T}_b} | \hat{u}_\theta (\mathbf{x})|^2.
    \end{align*}
...
\begin{mdframed}
    \begin{exmp}\label{exmp:fallo}
        ...
        Bajo esta configuración, obtenemos la función de red $\hat{u}_\theta(\mathbf{x})$ numéricamente a través del método PINN y la comparamos con la solución débil en la Figura \ref{fig:deviation}. 
...
Por otro lado, el método Deep Ritz plantea una alternativa más alineada con la formulación variacional de los problemas elípticos, minimizando un funcional en lugar de imponer restricciones puntuales. Sin embargo, el uso de penalizaciones para imponer condiciones de contorno introduce modificaciones en el problema original, lo que afecta a la precisión de la solución obtenida. A pesar de ello, este enfoque podría ser más viable en problemas de alta dimensionalidad, donde los métodos tradicionales pierden eficiencia.
...
La función \texttt{main} define los parámetros, inicializa la red, ejecuta el entrenamiento y evaluación y grafica los resultados. Los parámetros utilizados tienen el siguiente significado:
\begin{itemize}
