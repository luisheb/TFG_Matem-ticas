...
def train(model, device, params, optimizer, scheduler):
    ...
    data1 = (
        torch.from_numpy(generateData.sampleFromSquare(1, params["bodyBatch"]))
        .float()
        .to(device)
    )
    data2 = (
        torch.from_numpy(generateData.sampleFromSquareBoundary(1, params["bdryBatch"]))
        .float()
        .to(device)
    )
    ...
    for step in range(params["trainStep"] - params["preStep"]):
        ...
        if step % params["sampleStep"] == params["sampleStep"] - 1:
            ...
            data2 = (
                torch.from_numpy(
                    generateData.sampleFromSquareBoundary(1, params["bdryBatch"])
                )
                .float()
                .to(device)
            )
...
def test(model, device, params):
    ...
    data = (
        torch.from_numpy(generateData.sampleFromSquare(1, numQuad)).float().to(device)
    )
    output = model(data)
