...
def sampleFromBoundary(n):
    ...
    for i in range(n):
        ...
        point5 = np.array(
            [
                c[0] + r * math.cos(2 * math.pi * rand1),
                c[1] + r * math.sin(2 * math.pi * rand1),
            ]
        )

        array[i] = (
            myFun(rand0, interval1) * point1
            + myFun(rand0, interval2) * point2
            + myFun(rand0, interval3) * point3
            + myFun(rand0, interval4) * point4
...
def sampleFromSquareBoundary(a, n):
    ...
    samples[mask0] = np.column_stack(
        (positions[mask0], np.full(np.sum(mask0), a))
    )  # Top edge
    samples[mask1] = np.column_stack(
        (positions[mask1], np.full(np.sum(mask1), 0))
    )  # Bottom edge
    samples[mask2] = np.column_stack(
        (np.full(np.sum(mask2), a), positions[mask2])
    )  # Right edge
    samples[mask3] = np.column_stack(
        (np.full(np.sum(mask3), 0), positions[mask3])
    )  # Left edge
...
def sampleFromLShapeBoundary(a, n):
    ...
    while len(samples) < n:
        ...
        x, y = (1 - t) * np.array([x1, y1]) + t * np.array(
            [x2, y2]
