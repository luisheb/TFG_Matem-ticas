...
def train(model,device,params,optimizer,scheduler):
    ...
    data1 = torch.from_numpy(generateData.sampleFromLShape(1,params["bodyBatch"])).float().to(device)
    data2 = torch.from_numpy(generateData.sampleFromLShapeBoundary(1,params["bdryBatch"])).float().to(device)
...
def test(model, device, params):
    ...
    data = torch.from_numpy(generateData.sampleFromLShape(1,numQuad)).float().to(device)
    output = model(data)
